...
const AssignTrainerScreen = () => {
    ...
    const confirmAssignment = async () => {
      ...
      try {
        ...
        if (response?.status === 200) {
          ...
          Alert.alert(
            "Error",
            response?.detail || `Failed to assign ${assignmentType}`
          );
        }
      } catch (error) {
  ...
  const fetchAssignments = async () => {
    ...
    try {
      ...
      if (response?.status === 200) {
        ...
        Alert.alert("Error fetching assignments");
      }
    } catch (error) {
      alert("Error fetching Assignments");
    } finally {
  ...
  const fetchPlansAndBatches = async () => {
    ...
    try {
      const gymId = await getToken("gym_id");
      if (!gymId) {
        alert("Error fetching plans and batches");
        return;
      }
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.scrollContainer}>
        ...
        {!assignmentType && (
          <Text style={styles.title}>Select the type of assignment</Text>
        )}

        <View style={styles.section}>
          <View style={styles.radioGroup}>
            {["trainer", "workout", "diet"].map((type) => {
              const isDisabled =
                (type === "trainer" && trainers.length === 0) ||
                (type === "workout" && workoutPlans.length === 0) ||
                (type === "diet" && dietPlans.length === 0);
              ...
              return (
                <TouchableOpacity
                  ...
                  style={[
                    styles.radioButton,
                    assignmentType === type && styles.radioSelected,
                ...
                >
                  <Text
                    style={[
                      styles.radioText,
                      assignmentType === type && styles.radioTextSelected,
